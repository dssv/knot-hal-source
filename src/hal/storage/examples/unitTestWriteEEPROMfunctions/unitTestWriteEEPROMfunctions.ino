...
static uint8_t value_PUBLIC_KEY[PUBLIC_KEY_SIZE] = {
	0x1F, 0x37, 0x66, 0xC9, 0x8A, 0xDB, 0x2D, 0x0C, 0xAF, 0x8C, 0x02, 0xE4,
	 0x47, 0x05, 0x9C, 0xB0, 0xFE, 0x6F, 0x1A, 0x40, 0x3D, 0x57, 0x70, 0x0D,
	  0x9B, 0x3F, 0x34, 0xCD, 0xD4, 0xCF, 0xDC, 0x8C, 0x36, 0x6D, 0x48, 0x58,
	   0xAD, 0x64, 0xE5, 0xE4, 0xAE, 0x9A, 0xC1, 0x49, 0xEF, 0xD9, 0x94, 0x75,
	    0xF8, 0x24, 0x7B, 0x9A, 0xCF, 0x18, 0x53, 0xF9, 0x41, 0xB0, 0xC5, 0x5A,
	     0x86, 0xB9, 0x60, 0xF4
};
...
static uint8_t value_FOREIGN_KEY[FOREIGN_KEY_SIZE] = {
	0x06, 0xE6, 0x9C, 0xFE, 0x72, 0x2A, 0xAC, 0x71, 0x55, 0xA3, 0x4A, 0x9A,
	 0xD1, 0x6B, 0x21, 0xA7, 0x51, 0xCC, 0xE8, 0xA0, 0x5C, 0x65, 0xC2, 0xA8,
	  0x77, 0xDD, 0x9D, 0x4C, 0x3D, 0xDC, 0x7A, 0x03, 0x84, 0x7E, 0x37, 0xC3,
	   0x08, 0x70, 0x17, 0xDB, 0xCD, 0xF1, 0x31, 0xD0, 0x79, 0xEF, 0x2E, 0xB0,
	    0xF2, 0x09, 0xBA, 0xDF, 0x57, 0xE8, 0xA5, 0x3D, 0x47, 0xE1, 0x1D, 0x42,
	     0x1B, 0x32, 0x3F, 0x96
};
...
void setup()
{
	...
	hal_storage_write_end(HAL_STORAGE_ID_PRIVATE_KEY, (void *) value_PRIVATE_KEY, sizeof(value_PRIVATE_KEY));
	hal_storage_write_end(HAL_STORAGE_ID_PUBLIC_KEY, (void *) value_PUBLIC_KEY, sizeof(value_PUBLIC_KEY));
	hal_storage_write_end(HAL_STORAGE_ID_FOREIGN_KEY, (void *) value_FOREIGN_KEY, sizeof(value_FOREIGN_KEY));
	...
	eeprom_read_block(&value_PRIVATE_KEY_out, (const void *) ADDR_PRIVATE_KEY, sizeof(value_PRIVATE_KEY_out));

	eeprom_read_block(&value_PUBLIC_KEY_out, (const void *) ADDR_PUBLIC_KEY,
	 sizeof(value_PUBLIC_KEY_out));

	eeprom_read_block(&value_FOREIGN_KEY_out, (const void *) ADDR_FOREIGN_KEY, sizeof(value_FOREIGN_KEY_out));
	...
	value_PRIVATE_KEY[sizeof(value_PRIVATE_KEY)] = '\0';
	value_PRIVATE_KEY_out[sizeof(value_PRIVATE_KEY_out)] = '\0';
	...
	value_PUBLIC_KEY[sizeof(value_PUBLIC_KEY)] = '\0';
	value_PUBLIC_KEY_out[sizeof(value_PUBLIC_KEY_out)] = '\0';
	printf("PUBLIC KEY: \t\t'%s'\n", value_PUBLIC_KEY);
	printf("PUBLIC KEY read:\t'%s'\n\n", value_PUBLIC_KEY_out);

	/* FOREIGN KEY */
	if ((memcmp(value_FOREIGN_KEY, value_FOREIGN_KEY_out, sizeof(value_FOREIGN_KEY))) == 0)
		printf("FOREIGN KEY: \t\tOk\n");
	else
		printf("Problems reading FOREIGN KEY\n");
	value_FOREIGN_KEY[sizeof(value_FOREIGN_KEY)] = '\0';
	value_FOREIGN_KEY_out[sizeof(value_FOREIGN_KEY_out)] = '\0';
	printf("FOREIGN KEY: \t\t'%s'\n", value_FOREIGN_KEY);
	printf("FOREIGN KEY read:\t'%s'\n\n", value_FOREIGN_KEY_out);
